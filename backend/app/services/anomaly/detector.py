...
class AnomalyDetector:
    ...
    def isolation_forest(
        ...
        return [True if x == -1 else False for x in prediction]
...
if __name__ == "__main__":
    ...
    sales_df["is_anomaly"] = AnomalyDetector.detect(sales_values, method="z_score")
    ...
    inventory_values = inventory_df["closing_stock"].tolist()

    inventory_df["is_anomaly"] = AnomalyDetector.detect(
        inventory_values, method="z_score"
    )
    ...
    sales_df.to_excel("artifacts/sales_anomaly_result.xlsx", index=False)

    inventory_df.to_excel("artifacts/inventory_anomaly_result.xlsx", index=False)
