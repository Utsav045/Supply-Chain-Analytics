...
def main():
    ...
    sales_df = add_lag_features(sales_df, "date")

    print("Generating Lag Features for Inventory...")
    inventory_df = add_lag_features(inventory_df, "date")

    print("Generating Lag Features for Supplier...")
    supplier_df = add_lag_features(supplier_df, "delivery_date")

    print("Saving Datasets...")

    sales_df.to_csv("data/processed/lag_sales_data.csv", index=False)

    inventory_df.to_csv("data/processed/lag_inventory_data.csv", index=False)

    supplier_df.to_csv("data/processed/lag_supplier_data.csv", index=False)
