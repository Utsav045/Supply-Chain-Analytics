...
def create_forecasting_features(
    ...
    if "demand" not in dataframe.columns:
        raise ValueError(
            "Forecasting features require a demand column."
        )
    ...
    featured["week_of_year"] = (
        featured.index.isocalendar().week.astype(int)
    )
    ...
    for lag in (1, 7, 14, 28):
        featured[f"demand_lag_{lag}"] = (
            featured["demand"].shift(lag)
        )

    for window in (7, 14, 28):
        featured[f"demand_rolling_mean_{window}"] = (
            shifted_demand.rolling(
                window=window,
                min_periods=window,
            ).mean()
        )
    ...
    if "inventory_level" in featured.columns:
        ...
        featured["is_stockout"] = (
            featured["inventory_level"].fillna(0) <= 0
        ).astype(int)
        ...
        featured["inventory_change"] = (
            featured["inventory_level"].diff()
        )
    ...
    if "price" in featured.columns:
        ...
        featured["price_change"] = (
            featured["price"].pct_change(fill_method=None)
        )
    ...
    if "promotion" in featured.columns:
        ...
        featured["promotion_lag_1"] = (
            featured["promotion"].shift(1)
        )
